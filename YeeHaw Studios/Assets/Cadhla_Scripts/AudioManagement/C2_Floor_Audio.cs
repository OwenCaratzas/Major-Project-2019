...
public class C2_Floor_Audio : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Player")
        {
            ...
            {
                _C2_GroundSentry[i].volume = 0.1f;
            }

            for (int i =0; i<_C2_GroundSentry_Check.Count; i++)
            {
                _C2_GroundSentry_Check[i].detectionActive = false;
            }


            for (int i = 0; i < _C2_UpstairsSentry.Count; i++)
            {
                _C2_UpstairsSentry[i].volume = 0.5f;
            }

            for (int i = 0; i<_C2_UpstairsSentry_Check.Count; i++)
            {
                _C2_UpstairsSentry_Check[i].detectionActive = true;
            }


            for (int i = 0; i < _C2_UpstairsFence.Count; i++)
            {
                _C2_UpstairsFence[i].volume = 0.4f;
            }
        }
    }
}
